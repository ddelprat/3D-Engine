...
public:
	...
	double intersect(const Ray& r, Vector& P, Vector& N) {
		...
		if (t2 < 0) {
			return -1; // la sphere est derrière nous
		}
		double t;
		if (t1>0) {
			t = t1;
...
public:
	...
	double first_intersect(const Ray& r, Vector& P, Vector& N, Vector& Albedo, bool& Mirror, bool& isTransparent) {
	...
	Vector getColor(const Ray& r, int nb_rebound) {
		Vector P, N;
		Vector Albedo;
		...
		double min_dist = first_intersect(r, P, N, Albedo, Mirror, isTransparent);
		if (min_dist != -1) {
			if (Mirror) {
			...
			if (isTransparent) {
				double n1 = 1.;
				// affecter le paramètre comme attribut de la sphère
				double n2 = 1.4;
				Vector Ntransp = N;
				if (dot(r.u, N) > 0) {
					std::swap(n1,n2);
			...
			Vector PL = L - P;
			double PL_norm2 = PL.norm2();
			PL.normalize();
			Ray shadowRay(P+0.0001*N,PL);
			Vector shadowColor, shadowN, shadowP;
			if (first_intersect(shadowRay, shadowP, shadowN, shadowColor, Mirror, isTransparent) < sqrt(PL_norm2)) {
				return Vector(0, 0, 0);
			}
			return (I * std::max(0.0, dot(N, PL)) * Albedo) / (4 * M_PI * PL_norm2);
		}
		return Vector(0, 0, 0);
	}
...
int main() {
	...
	scene.I = 2e7;
	scene.addSphere(Sphere(Vector(0,1050,0), 1000, Vector(0.5,0.5,1)));
	scene.addSphere(Sphere(Vector(0,0,1100), 1000, Vector(0.5,1,0.5)));
	scene.addSphere(Sphere(Vector(0,-1010,0), 1000, Vector(0.5,0.5,1)));
	scene.addSphere(Sphere(Vector(0,0,-1100), 1000, Vector(1,0.5,0.5)));
	scene.addSphere(Sphere(Vector(1050,0,0), 1000, Vector(0.5,0.5,1)));
	scene.addSphere(Sphere(Vector(-1050,0,0), 1000, Vector(0.5,0.5,1)));
	scene.addSphere(Sphere(Vector(0,0,0),10, Vector(0.5,0.5,0.5),true, false, 1.5));
	Vector O(0,0,-55);
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			for (int k = 1; k < num_simu; k++) {
				...
				double n2 = rr * cos(2.0 * M_PI * u2) * 0.5;
			...
			// Cap values over 255
			image[(i*W + j) * 3 + 0] = std::pow(std::max(0.,std::min(255.,color[0]))/255,1/2.2)*255;  // RED
			image[(i*W + j) * 3 + 1] = std::pow(std::max(0.,std::min(255.,color[1]))/255,1/2.2)*255;  // GREEN
			image[(i*W + j) * 3 + 2] = std::pow(std::max(0.,std::min(255.,color[2]))/255,1/2.2)*255;  // BLUE
		}
	}
